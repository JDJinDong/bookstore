...
def order_commit(request):
	...
	pay_method = request.POST.get('pay_methond')
...
# 		alipay_public_key_path = os.path.join(settings.BASE_DIR,'order/alipay_public_key.pem'),#支付宝的公钥，验证支付宝回传消息使用
...
# 			subject = '书店商城%s'%order_id,
# 			return_url = None,
# 			notify_url = None #可选，不填则默认使用notify url
# 	)
# 	#返回应答
...
# 			order.save()
# 			#返回数据
# 			return JsonResponse({'res':3,'massage':'支付成功'})
...
# 		else:
# 			#支付出错
# 			return JsonResponse({'res':4,'errmag':'支付出错'})
#
